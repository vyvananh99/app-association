...
const JobTypeDeleteConfirm = ({ open, setOpen, onConfirm, data }) => {
  const { id, name, pay } = data || {};
  ...
  return (
    <Dialog open={open} onClose={() => setOpen(false)} maxWidth="xs" fullWidth>
      ...
      <DialogContent>
        <Typography>
          <strong>Allow Type Information:</strong>
        </Typography>
        <div style={{ marginTop: 10 }}>
          ...
          <div>
            <span style={{ display: 'inline-block', width: '150px' }}>Pay</span>
            <span style={{ paddingRight: 10 }}>:</span>
            <span> {pay || 'N/A'}</span>
          </div>
        </div>
        <Typography mt={2}>Are you sure you want to delete this Allow Type?</Typography>
      </DialogContent>
      <DialogActions>
...
export default JobTypeDeleteConfirm;
