...
import CityDeleteConfirm from './city-delete-confirm';
import { Refresh, Add, Edit, Delete } from '@mui/icons-material';

import { getCityApi, updateCityApi, createCityApi, deleteCityApi } from 'api/city.api';
...
const City = () => {
  ...
  const fetchData = async () => {
    try {
      const cityData = await getCityApi();
  ...
  const onConfirmDelete = async (data) => {
    try {
      const deleteResult = await deleteCityApi(data.id);
  ...
  const onAction = async (data) => {
    try {
      if (action === 'create') {
        const resultCreate = await createCityApi(data);
        if (resultCreate.isSuccess) toast.success('Create City successfully');
        else toast.error(resultCreate.message);
      } else if (action === 'edit' && currentId) {
        const resultUpdate = await updateCityApi(data, currentId);
        if (resultUpdate.isSuccess) toast.success('Update City successfully');
      ...
      toast.error(`${action === 'edit' ? 'Update' : 'Create'} City failed`);
    }
  };
  ...
  return (
    <Grid container spacing={2} alignItems="center">
      ...
      <CommonTable data={cityData} columns={columns} searchQuery={searchQuery} tableHeight={310} />
      <CityModal open={open} setOpen={setOpen} rowData={rowData} onAction={onAction} action={action} />
      <CityDeleteConfirm open={openDeleteConfirm} setOpen={setOpenDeleteConfirm} onConfirm={onConfirmDelete} data={rowData} />
    </Grid>
  );
};
