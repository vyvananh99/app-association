...
const LocationModal = ({ open, setOpen, onAction, rowData, action }) => {
  const handleClose = () => setOpen(false);

  const validationSchema = Yup.object({
    id: Yup.string()
      .required('Province Code is required')
      .matches(/^[A-Za-z0-9]{1,3}$/, 'Province Code must be 1 characters and less than 4 characters'),

    vnName: Yup.string().required('Province English Name is required').max(255, 'Province Name cannot be longer than 255 characters'),

    enName: Yup.string().required('Province Vietnamese Name is required').max(255, 'Vietnamese Name cannot be longer than 255 characters')
  });
    ...
    <Modal open={open} onClose={handleClose}>
      ...
      >
        ...
        <Typography variant="h4" sx={{ marginBottom: 2 }}>
          {action === 'edit' ? 'Update Province' : 'Create Province'}
        </Typography>
        ...
        >
          {({ values, handleChange, errors, touched }) => (
            <Form style={{ width: '100%' }}>
              ...
              >
                <Button type="submit" variant="contained" sx={{ width: 80, height: 30 }}>
